  ...
  useEffect(() => {
    const initializeData = async () => {
      ...
      try {
        ...
        if (!connected) {
          console.error('Failed to connect to Supabase. Some features may not work properly.');
        }
      } catch (error) {
...
export default function InterviewStartPage() {
  ...
  const handleStartInterview = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!connected) {
        throw new Error('Cannot start interview due to database connection issues');
      }
      ...
      window.location.href = `#session=${sessionId}&candidate=${candidateId}`;
